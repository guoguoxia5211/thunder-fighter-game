...
function addTankEditorStyles() {
  ...
  style.textContent = `
    #tank-editor-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 9999; overflow-y: auto; padding: 20px; }
    .tank-editor-container { background: white; border-radius: 15px; max-width: 600px; margin: 20px auto; overflow: hidden; }
    .tank-editor-header { background: linear-gradient(135deg, #1a1a2e, #16213e); color: white; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; }
    .tank-editor-header h2 { margin: 0; font-size: 18px; }
    .tank-close-btn { background: rgba(255,255,255,0.2); border: none; color: white; padding: 8px 15px; border-radius: 6px; cursor: pointer; font-weight: bold; }
    .tank-category-filter { padding: 15px; background: #f8f9fa; display: flex; gap: 8px; flex-wrap: wrap; }
    .tank-filter-btn { padding: 8px 15px; border: none; border-radius: 20px; background: #e0e0e0; cursor: pointer; font-size: 13px; }
    .tank-filter-btn.active { background: linear-gradient(135deg, #667eea, #764ba2); color: white; }
    .tank-editor-content { padding: 20px; max-height: 50vh; overflow-y: auto; }
    .tank-resource-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 15px; }
    .tank-resource-item { background: #f8f9fa; border-radius: 10px; padding: 15px; text-align: center; border: 2px solid transparent; transition: all 0.3s; }
    .tank-resource-item:hover { border-color: #667eea; transform: translateY(-3px); }
    .tank-resource-preview { width: 60px; height: 60px; object-fit: contain; margin: 0 auto 10px; border: 2px solid #ddd; border-radius: 8px; background: white; }
    .tank-resource-name { font-size: 13px; color: #2c3e50; margin-bottom: 10px; }
    .tank-upload-btn { background: linear-gradient(135deg, #3498db, #2980b9); border: none; padding: 8px 15px; color: white; border-radius: 6px; cursor: pointer; font-size: 13px; }
    .tank-editor-footer { background: #f8f9fa; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #e0e0e0; }
    .tank-status { color: #27ae60; font-weight: bold; }
    .tank-actions { display: flex; gap: 10px; }
    .tank-btn { padding: 10px 20px; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 14px; }
